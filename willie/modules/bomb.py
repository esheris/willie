...
def cutwire(bot, trigger):
    ...
    if wirecut.lower() in ('all', 'all!'):
        ...
        kmsg = ('Cutting ALL the wires! *boom* (You should\'ve picked the %s wire.)'
                % (color))
        bot.say([kmsg])
        ...
        sch.cancel(code)  # defuse timer, execute premature detonation
        kmsg = 'No! No, that\'s the wrong one. Aww, you\'ve gone and killed yourself. Oh, that\'s... that\'s not good. No good at all, really. Wow. Sorry. (You should\'ve picked the ' + color + ' wire.)'
        bot.say([kmsg])


def explode(bot, trigger):
    target = trigger.group(1)
    kmsg = 'Oh, come on, ' + target + '! You could\'ve at least picked one! Now you\'re dead. Guts, all over the place. You see that? Guts, all over YourPants. (You should\'ve picked the ' + bombs[target.lower()][0] + ' wire.)'
    bot.say([kmsg])
